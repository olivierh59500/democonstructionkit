...
// Wave is a spatial sine wave traveling with angular speed in radians/second.
type Wave struct{ Amplitude, Spatial, Speed, Phase float64 }

// At samples a wave at a position and an absolute time.
func (w Wave) At(position, seconds float64) float64 {
	return w.Amplitude * math.Sin(position*w.Spatial+seconds*w.Speed+w.Phase)
}
