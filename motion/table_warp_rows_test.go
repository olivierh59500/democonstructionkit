...
func TestTableWarpRowsPreserveFlatEhhhLogoProfile(t *testing.T) {
	curve := []float64{0, 20, -20, 4}
	program, err := NewTableWarpRows(TableWarpRowsConfig{
		Curve: curve, Rows: 170, Step: 1,
		XBase: 384, HalfWidth: 383.5, RowOffset: -.5,
		SourceHeight: 1, BounceDivisor: 1,
	})
	if err != nil {
		t.Fatal(err)
	}
	for tick := 0; tick < 1000; tick++ {
		for row := 0; row < 170; row++ {
			want := SampledRowPose{SourceY: float64(row), SourceHeight: 1,
				X: 384 + curve[(tick+row)%len(curve)] - 383.5,
				Y: float64(row) - .5, ScaleX: 1, ScaleY: 1}
			if got := program.Sample(0, 0, row); got != want {
				t.Fatalf("tick %d row %d = %+v, want %+v", tick, row, got, want)
			}
		}
		if program.Bounce() != 0 {
			t.Fatalf("tick %d flat logo gained a bounce", tick)
		}
		program.Advance()
	}
}
