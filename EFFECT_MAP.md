...
| Starwars | `Config.Crawl`, RowProjection, scrolling, `sprites.ProjectedField`, `RasterOverlay` source-in fill, rectified `motion.Wave` profile and `sprites.SampledSpriteTrain` over one authored XY path | The dual-color wave-strip text material and source flash cue remain screen composition. |
