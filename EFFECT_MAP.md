...
| Mega Scroller | `composite.TiledWaveBackdrop` for the cached two-wave tile field, scrolling, directional `motion.BounceBank` text-surface transport and a reusable `RasterOverlay` source-atop bar mask | Authored text, bar artwork and scene layer order remain production data. |
