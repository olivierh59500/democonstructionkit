...
| Phenomena DNA intro | Atlas, DNAFrames, `scrolling.SliceProgram` with two-pixel insertion, loop-start and configurable control cues, `motion.RecurrentRowWave` baseline, and `motion.GravityBounce` photon path | Intro/outro reveal and fade cues remain screen composition. |
