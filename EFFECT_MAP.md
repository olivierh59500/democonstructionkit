...
| Phenomena DNA intro | Atlas, DNAFrames, sliced transport and `motion.RecurrentRowWave` for its staggered cosine baseline | Whole screen's two-pixel insertion, loop-start and control events as a reusable configuration; separate intro/outro reveal and bounce cues. |
