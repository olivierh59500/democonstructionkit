...
| `grodan-kvack-kvack-demo` | Four scroll lanes use `Config.Ribbon`; its twelve-sprite chain uses `motion.HarmonicFormation` through `sprites.Group`. |
...
| Grodan | Atlas, four `scrolling.Config.Ribbon` lanes, bounded `Background` repetition and harmonic `sprites.Group` with bounce envelope | Authored raster fills and scene layer order. |
