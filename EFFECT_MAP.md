...
| Spreadpoint | `Config.Bands`, feedback DNA, atlas, a 20-ball `sprites.Group` with pixel-snapped formula, a dynamic `SurfaceLayer` for the raster-filled logo and `motion.PhaseSequence` with `motion.HarmonicTransform` for its orbit/zoom | Card/audio cue sequence and authored layer order. |
