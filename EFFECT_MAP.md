...
| Coco, main | `Config.Scanline`, `SolidCubeTrain`, sprites.Group grid/translation, shared font metrics, `CopperBars`, `RotozoomBackground` source quad and `SurfaceLayer` with a moving title pass | Authored art and scene layer order. |
...
| Phenomena DNA intro | Atlas, DNAFrames, `scrolling.SliceProgram` with two-pixel insertion, loop-start and configurable control cues, `motion.RecurrentRowWave` baseline, `motion.GravityBounce` photon path, `motion.WrapBank` hue clock, `timeline.ScalarStages` intro/outro thresholds, and reusable gradient/silhouette/inverted-font materials | Visual reveal/fade materials and layer order remain screen composition. |
...
| Ehhh | Scrolling, compiled row-profile program shared with Digi, `sprites.Train` raster bars with phase-spaced cosine `motion.Wave`, and `motion.CuedWaveClock` for lookahead characters, sticky text lanes and stop-at-landing roller timing | The logo row placement and inner-roller strip movement remain screen composition. |
| Mega Scroller | Tiled background, WaveStrips, scrolling, directional `motion.BounceBank` text-surface transport and a reusable `RasterOverlay` source-atop bar mask | Authored text, bar artwork and scene layer order remain production data. |
| Spreadpoint | `Config.Bands`, feedback DNA, atlas, a 20-ball `sprites.Group` with pixel-snapped formula and a dynamic `SurfaceLayer` for the raster-filled logo | Card/audio cue sequence and authored layer order. |
...
| LED Scroller | Bounded tiled background, cached bubble matrix, scrolling, `motion.WrapBank` backdrop/gradient offsets, harmonic `sprites.Group`, rectified `motion.WaveClock` and `palette.UniformGradient` 11-color raster material | Authored artwork, initial background preload and layer order remain scene composition. |
...
| Reset | `Config.Slots`, scrolling, atlas, `timeline.StageSequence` with immediate events and cue windows, `composite.PairedRasterOrbit` for ordered pairs and `motion.WrapBank` for vertical raster fill | Source-atop compositing and the authored backdrop trajectory remain screen composition. |
