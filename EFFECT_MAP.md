...
| Phenomena DNA intro | Atlas, DNAFrames, `scrolling.SliceProgram` with two-pixel insertion, loop-start and configurable control cues, `motion.RecurrentRowWave` baseline, `motion.GravityBounce` photon path, `timeline.ScalarStages` intro/outro thresholds, and reusable gradient/silhouette/inverted-font materials | Visual reveal/fade materials, hue clock and layer order remain screen composition. |
