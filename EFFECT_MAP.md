...
| Starwars | `Config.Crawl`, `scrolling.DualProfiledRing` with two independent fonts, segmented wave profile, source-in raster and exact first-frame filtering, `sprites.ProjectedField`, and `sprites.SampledSpriteTrain` over one authored XY path | The background flash cue and layer placement remain scene composition. |
