...
| Phenomena DNA intro | Atlas, DNAFrames, `scrolling.SliceProgram` with two-pixel insertion, loop-start and configurable control cues, `motion.RecurrentRowWave` baseline, `motion.GravityBounce` photon path, and reusable gradient/silhouette/inverted-font materials | Intro/outro reveal and fade cues remain screen composition. |
