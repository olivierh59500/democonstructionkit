...
| DOM intro | Atlas, `scrolling.Config.SizeBank` with `motion.ScaledTextClock`, `motion.WrapBank` background/raster offsets and `sprites.AnimatedField` star atlas instances | Authored text, raster masking and scene layer order. |
